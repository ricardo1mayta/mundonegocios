...
export class NuevaVentaComponent {
  ...
  constructor(private dialog: MatDialog) {
    // Inicializa productos manualmente
    this.inventarioService.listaProductosDisponibles().subscribe({
      next: (response: any) => {
        this.productos.set(response.data as Inventario[]);
      },
      error: err => {
    ...
    if (cotizacionId) {
      console.log('Editing quotation with ID:', cotizacionId);
  ...
  private cargarCotizacion(id: number) {
    this.cotizacionesService.obtenerPedidoPorId(id).subscribe((comp: any) => {
      const cotizacion = comp.data;
      // Llena el formulario:
      this.selectedProvider.set(cotizacion.cliente);
      this.cart.set(cotizacion.pedidosDetalle);
